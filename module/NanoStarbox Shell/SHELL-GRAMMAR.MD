...
and differences are between these main (4) sub contexts. Each context also
has full access to its parent context, but not all contexts require the
functionality. Historically functions must have access to local and
parent environment.

Java based environment objects have no context interface, but they 
could be integrated by an implementation, that chooses to notify the 
object of the current context upon execution, or by creating an object
with a reference to some super context, which the current context
derives from.

Each sub context is an object which derives from a single main context,
so to each child context, there is no difference in context which
affects the operational performance of execution, and there is no
difference in environment manipulation or access techniques among the 
various contexts.

Whenever an evaluation or expansion is required for an execution, such
as in the case of parameters, the interpreter will be provided with the
context of the caller, for filling in the data. This is "normal" shell
operation, meaning that the calling context (whatever that notion may 
be externally) always fills in the parameters for the exec call.

Background command executions execute in a Command Shell Context (details below) as a
java thread. Java doesn't play well with process handles out of the box
so, strategy on implementation is less than forthcoming at this time,
with looming questions for shell job control such as: `bg, fg, and kill`.
Normally, these integrations are not used in scripting, but in interactive
sessions.

Pipe execution status is a question, as well as which method will be
used to get the return value of a background command. With the advent
of java objects, these facilities may not be needed, because custom
objects, functions and commands can emulate the job control layer.

A context can programmatically do, everything a script can do, such
as run commands, evaluate statements and expressions, access environment
and configuration, etc, which makes it convenient to write a compiler,
optimized command form or script interpreter backend with seamless
integration.

It is noted that this performance model may not be compatible with the
desired implementation of command shell, and that this is what the
defacto implementation provides, having an affinity for running shell
styled programs with hassle-free java integrations. If your script interpreter
uses a different scheme, you will need these tables if you hope to write
a compatible script translation.
<br><br>
```
...
[X] has parent environment OR
[ ] has parent environment (copy)
[X] has children
...
```
Source Command Context Properties:
